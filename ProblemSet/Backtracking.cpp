...
vector<string> Backtracking::addOperators(string num, int target) {
    vector<string> res;
    addOperatorsHelper(num, target, 0, 0, "", res);
    return res;
}
void Backtracking::addOperatorsHelper(string num, int target, long diff, long curNum, string out, vector<string>& res) {
    ...
    for (int i = 1; i <= num.size(); ++i) {
        ...
        if (out.size() > 0) {
            addOperatorsHelper(next, target, stoll(cur), curNum + stoll(cur), out + "+" + cur, res);
            addOperatorsHelper(next, target, -stoll(cur), curNum - stoll(cur), out + "-" + cur, res);
            addOperatorsHelper(next, target, diff * stoll(cur), (curNum - diff) + diff * stoll(cur), out + "*" + cur, res);
        } else { // first operation
            addOperatorsHelper(next, target, stoll(cur), stoll(cur), cur, res);
        }
    }
}
